...
namespace Shared.Http
{
    public static class HttpResult
    {
        public static async Task<Result<TResponse>> CatchResponseAsync<TResponse>(Func<CancellationToken, Task<HttpResponseMessage>> func, JsonSerializerOptions options, CancellationToken ct = default)
        {
            ...
            try
            {
                ...
                if (response.StatusCode == HttpStatusCode.NoContent || response.Content.Headers.ContentLength == 0)
                    return new Error(AppErrors.BackendHttp, $"Пустое тело ответа при ожидаемом типе {typeof(TResponse).Name}");
            ...
            {
                return new Error(AppErrors.BackendHttp, $"Timeout: {ex.Message}");
            }
            catch (JsonException ex)
            {
                return new Error(AppErrors.BackendHttp, ex.Message);
            }
        ...
        public static async Task<Result<Nothing>> CatchAsync(Func<CancellationToken, Task<HttpResponseMessage>> func, CancellationToken ct = default)
        {
            try
            {
